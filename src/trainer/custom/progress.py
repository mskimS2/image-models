...
from trainer.callbacks import Callback
...
class Progress(Callback):
    ...
    def on_train_start(self, trainer, **kwargs):
        self._train_tqdm = tqdm(
            total=self.num_train_steps, leave=False, ncols=100)

    def on_valid_epoch_start(self, trainer, **kwargs):
        if self.num_valid_steps:
            self._valid_tqdm = tqdm(
                total=self.num_valid_steps, leave=False, ncols=100)

    def format_metrics(self, metrics, stage):
        metrics_str = ", ".join(["{}={:.4f}".format(
            k, v) for k, v in metrics.items() if k not in ("epoch", "steps")])
        if stage == "train":
    ...
    def on_train_step_end(self, trainer, **kwargs):
        train_metrics = trainer.metrics["train"]
        if "epoch" in train_metrics:
            del train_metrics["epoch"]
        self._train_tqdm.set_postfix(
            epoch=trainer.current_epoch, **train_metrics)
    ...
    def on_valid_epoch_end(self, trainer, **kwargs):
        ...
        if len(trainer.metrics["valid"]) == 0:
            return

        train_metrics = trainer.metrics["train"]
        epoch = trainer.current_epoch
        steps = trainer._train_step
        train_metrics = self.format_metrics(train_metrics, stage="train")

        valid_metrics = trainer.metrics["valid"]
        valid_metrics["epoch"] = trainer.current_epoch
        valid_metrics = self.format_metrics(valid_metrics, stage="valid")

        metrics_string = f"{train_metrics} {valid_metrics} [epoch={epoch} steps={steps}]"
        self._valid_tqdm.write(metrics_string)

        metrics = {}
        metrics["epoch"] = epoch
        metrics["steps"] = steps
        metrics["train"] = {
            k: v for k, v in trainer.metrics["train"].items() if k not in ("epoch", "steps")}
        metrics["valid"] = {
            k: v for k, v in trainer.metrics["valid"].items() if k not in ("epoch", "steps")}
    ...
    def on_train_epoch_end(self, trainer, **kwargs):
        ...
        metrics["steps"] = steps
        metrics["train"] = {
            k: v for k, v in trainer.metrics["train"].items()
