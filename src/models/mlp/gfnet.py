...
class Block(nn.Module):
    def __init__(
        ...
        self.layers = nn.Sequential(
            ...
            DropPath(drop_path_p) if drop_path_p > 0.0 else nn.Identity(),
        )
...
class DownLayer(nn.Module):
    def __init__(self, img_size: int = 56, dim_in: int = 64, dim_out: int = 128) -> None:
...
class GFNet(nn.Module):
    def __init__(
        ...
        Args:
            ...
            drop_path_p (float): stochastic depth rate
        ...
        if uniform_drop:
            print("using uniform droppath with expect rate", drop_path_p)
            dpr = [drop_path_p for _ in range(depth)]  # stochastic depth decay rule
        else:
            print("using linear droppath with expect rate", drop_path_p * 0.5)
...
class GFNetPyramid(nn.Module):
    def __init__(
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_p, sum(depth))]  # stochastic depth decay rule
        cur = 0
        for i in range(4):
            h = sizes[i]
            w = h // 2 + 1
