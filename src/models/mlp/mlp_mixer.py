...
class FeedforwardLayer(nn.Module):
    ...
    def __init__(self, dim, mlp_dim=None, dropout_p: float = 0.0) -> None:
...
class MixerBlock(nn.Module):
    ...
    def __init__(
        self, n_patches: int, hidden_dim: int, tokens_mlp_dim: int, channels_mlp_dim: int, dropout_p: float
    ) -> None:
        super(MixerBlock, self).__init__()

        self.norm1 = nn.LayerNorm(hidden_dim)
        self.norm2 = nn.LayerNorm(hidden_dim)
        self.token_mlp_block = FeedforwardLayer(n_patches, tokens_mlp_dim, dropout_p)
        self.channel_mlp_block = FeedforwardLayer(hidden_dim, channels_mlp_dim, dropout_p)
